...
bash-docker:
	...
	## bash
	@docker run --rm -it -v $(CURDIR):/workspace --workdir /workspace $(DOCKER_IMAGE) bash
.PHONY: bash-docker

lint-docker:
	## Should: `make build-docker`
	## Run go get & lint & verify
	@docker run --rm -v $(CURDIR):/workspace --workdir /workspace $(DOCKER_IMAGE) bash -c "\
	echo '-> Run go get' \
